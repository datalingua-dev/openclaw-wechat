...
| ⏰ 定时任务 | 无 | **原生 Cron 引擎支持，由大模型自主管理与主动推送** |
| 🔒 数据隔离 | 无 | **多用户级物理数据隔离沙箱 (基于 WeCom_ID 智能分流)** |
...
- [x] 📎 文件消息发送 + 语音消息发送
- [x] 📝 图文消息(news)、文本卡片(textcard)、**Markdown 原生消息**推送
- [x] 📍 位置消息接收（经纬度 + 位置名称）
- [x] 🔗 链接分享消息接收
...
- [x] ⏰ **原生 Cron 定时引擎**（支持大模型自主设定周期性关怀任务并主动推送）
- [x] 🔒 **物理级数据沙箱隔离**（底层动态分配不同企微用户专属 `/users/<id>/` 目录存储交互文件，防止大模型幻觉泄露隐私）
...
#### 🏋️‍♂️ 健身私教模板与原生 Cron 引擎隔离 (Advanced Sandbox)

我们在项目中内置了一个极具代表性的高阶智能体模板：`agent-templates/fitness-coach/`。它展示了如何使大模型突破“被动一问一答”及“多用户内存互串”的瓶颈：

- **大模型自主 Cron 引擎**：搭载原生 `fitness-cron-plugin.js` 工具。用户可以说“以后每周一三五早上7点催我空腹有氧”，大模型理解后将自主生成后台调度任务。时间到达时，网关将**逆向唤醒**大模型，基于该用户状态主动推发拟人的定时关怀。
- **物理级多用户沙箱隔离**：内置了 Multi-User Data Sandbox 处理流。每个对话用户所产生的大模型持久化文件（档案 `profile.md`、状态表 `memory.md` 等），全部会被网关底层强行路由到特权专属的子文件夹中（如 `/workspace-fitness-coach/users/HuangHui/`）。即使大模型发生指令越狱，也绝对无法读取到群组或平台上其他并发用户的数据，达到了企业级隐私保障。
